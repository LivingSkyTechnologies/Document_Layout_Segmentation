...
def build(img_size, nclasses):
    ...
    x_a = Upsample(tensor=x_a, size=[img_size // 4, img_size // 4])
    x_a = tf.keras.layers.Dropout(0.5)(x_a)
    ...
    x_b = tf.keras.layers.Dropout(0.5)(x_b)
    ...
    x = Upsample(x, [img_size, img_size])
