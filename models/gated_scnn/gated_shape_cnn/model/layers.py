...
class AtrousPyramidPooling(tf.keras.layers.Layer):
    ...
    def call(self, x, training=None):
        ...
        img_net = tf.keras.layers.Dropout(0.5)(img_net)
        ...
        shape_net = tf.keras.layers.Dropout(0.5)(shape_net)
...
class Resnet50Backbone(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        ...
        self.backbone = None
        backbone = tf.keras.applications.ResNet50(input_shape=[None, None, 3], include_top=False)
        self.backbone = tf.keras.Model(
            backbone.input,
            outputs={
                ...
                's4': backbone.get_layer('conv5_block3_out').output,
            })
